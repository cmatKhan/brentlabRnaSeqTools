#'
#' Explore QC metrics and set thresholds
#' @description Interactively explore both user defined and nf-co/rnaseq_pipeline
#'              QC metrics. Additionally, explore possible batch effects by
#'              splitting data based on the sample_df input
#'
#' @import dplyr DT ggplot2 ggsci plotly readr RSQLite shiny shinydashboard stringr lubridate
#'
#' @param qc_database_path path to the sqlite database which contains the
...
#' @param sample_df_name name of the table which stores sample information, eg
#'                       id_column, libraryDate, condition
#' @param qc_metrics_df_name the name of the table which catalogs the metrics
#'                           which a user may threshold on, and the corresponding
#'                           comparative and 'status'
#' @param id_field the name of the common field between every table in the qc
#'                 database (except for the qc_metrics table, which is not a
#'                 table which is joined)
#' @param batch_choices a character vector, eg c("libraryDate", "libraryProtocol")
...
#' @param debug Logical. Default is false. Set to true to see more information
#'              in the console. Intended for development debugging
#'
#'
#'
#'@export
qcExplorer = function(qc_database_path, sample_df_name, qc_metrics_df_name,
                      id_field, batch_choices, debug = FALSE){

  DEBUG = debug
  QC_DATABASE_PATH = qc_database_path
  SAMPLE_DF = sample_df_name
  QC_METRICS_DF = qc_metrics_df_name
  ...
  ui = dashboardPage(
    ...
    dashboardSidebar(
      ...
      fluidRow(style = "padding-top:20px",
               column(12, align="center",
                      downloadButton("download_audited_samplesheet",
                                     "Audited Sample Sheet"))
      ),
      fluidRow(style = "padding-top:20px",
               column(12, align="center",
                      downloadButton("updated_status_sheet",
                                     "QC Metric Sheet"))
      )
    ),
    dashboardBody(
      tabItems(
        ...
        tabItem(tabName = "all_metrics",
          ...
          fluidRow(style = "padding-top:10px",
            ...
            box(width = NULL,
                downloadButton("download_distribution_plot_obj",
                               "Download R Plot Object")
            )
          ),
          fluidRow(style = 'padding-top:10px',
            box(title = "DataTable", solidHeader = TRUE, width = NULL,
                DT::dataTableOutput("histogram_sample_table"))
          )
        ),
        ## correlation_matrix -----------------------------------------------------
        tabItem(tabName = "correlation_matrix",
          ...
          fluidRow(style = "padding-top:10px",
            box(width = NULL,
                checkboxGroupInput(
                  inputId = "correlation_table_column_selector",
                  label = "Select Metrics To Display",
                  choices = NULL)
            )
          ),
          fluidRow(style = 'padding-top:10px',
            box(title = "DataTable", solidHeader = TRUE, width = NULL,
                DT::dataTableOutput("correlation_sample_table"))
          ),
          fluidRow(style = "padding-top:10px",
            box(width = NULL,
        ...
        tabItem(tabName = "scatter_plots",
          ...
          fluidRow(style = "padding-top:10px",
            box(width = NULL,
                checkboxGroupInput(
                  inputId = "scatterplot_table_column_selector",
                  label = "Select Metrics To Display",
                  choices = NULL)
            )
          ),
          fluidRow(style = "padding-top:10px",
            ...
            box(width = NULL,
                downloadButton("download_scatterplot_plot_obj",
                               "Download R Plot Object")
            )
          ),
          fluidRow(style = 'padding-top:10px',
            box(title = "DataTable", solidHeader = TRUE, width = NULL,
                DT::dataTableOutput("scatterplot_sample_table"))
          )
        )
      )
    )
  ...
  server = function(input, output) {
      ...
      updateCheckboxGroupInput(
        inputId = "histogram_table_column_selector",
      ...
      msg = "If you haven't already, it is a good idea to hit
      'reset sample audit' before doing anything else to ensure that the audit
      is accurate to your current status_df"
      ...
      qc_df = eventReactive(input$qc_table, {
        ...
        qc_database_connection = dbConnect(RSQLite::SQLite(), QC_DATABASE_PATH)
        df = dbReadTable(qc_database_connection,
                         input$qc_table, check.names = FALSE)
        dbDisconnect(qc_database_connection)
      ...
      qc_df_x = eventReactive(input$qc_table_x, {
        ...
        qc_database_connection = dbConnect(RSQLite::SQLite(), QC_DATABASE_PATH)
        df = dbReadTable(qc_database_connection,
                         input$qc_table_x, check.names = FALSE)
        dbDisconnect(qc_database_connection)
      ...
      qc_df_y = eventReactive(input$qc_table_y, {
        ...
        qc_database_connection = dbConnect(RSQLite::SQLite(), QC_DATABASE_PATH)
        df = dbReadTable(qc_database_connection,
                         input$qc_table_y, check.names = FALSE)
        dbDisconnect(qc_database_connection)

        df
      })

      distributions_table = eventReactive(c(input$histogram_table_column_selector,
      ...
      correlations_table = eventReactive(c(input$correlation_table_column_selector,
                                           reactive_values$sample_df), {
      ...
      scatterplot_table = eventReactive(c(input$scatterplot_table_column_selector,
                                          reactive_values$sample_df), {
      ...
      qc_metric_histogram = eventReactive(c(input$qc_metric,
        ...
        reactive_values$histogram_obj = metric_hist
      ...
      qc_metric_boxplot = eventReactive(c(input$distribution_plot_tab,
        ...
        if(input$distribution_plot_tab == 'boxplot'){
          ...
          df = qc_df() %>%
            left_join(reactive_values$sample_df, by=ID_FIELD)
          ...
          reactive_values$boxplot_obj = metric_boxplot
      ...
      scatter_plot = eventReactive(c(input$scatter_x,
         ...
         scatter_plot = qc_df_x() %>%
           left_join(qc_df_y(), by = ID_FIELD) %>%
           left_join(reactive_values$sample_df, by = ID_FIELD) %>%
           ggplot(aes(!!rlang::sym(x_var),
                      !!rlang::sym(y_var),
                      label = ID_FIELD, # NOTE: make this a global variable
                      label1 = ID_FIELD)) +
         ...
         reactive_values$scatterplot_obj = scatter_plot
  ...
  and then restart."
      ...
      output$histogram_sample_table = DT::renderDataTable({
      ...
      output$correlation_sample_table = DT::renderDataTable({
      ...
      output$scatterplot_sample_table = DT::renderDataTable({
      ...
      output$download_audited_samplesheet <- downloadHandler(
        ...
        content = function(file) {
          write_csv(reactive_values$sample_df, file)
        }
      )

      output$updated_status_sheet <- downloadHandler(
        filename = function() {
          timenow = str_replace(str_remove_all(lubridate::now(), "-|:"), " ", "_")

          paste0("qc_metrics_table_", timenow, ".csv")
        },
        content = function(file) {
      ...
      output$download_distribution_plot_obj = downloadHandler(
        filename = function() {
          timenow = str_replace(str_remove_all(lubridate::now(), "-|:"), " ", "_")
      ...
      output$download_scatterplot_plot_obj = downloadHandler(
        filename = function() {
          timenow = str_replace(str_remove_all(lubridate::now(), "-|:"), " ", "_")

          paste0("scatterplot_", input$scatter_x, "_", input$scatter_x,
                 "_", timenow, ".rds")
        },
        content = function(file) {
          readr::write_rds(reactive_values$scatterplot_obj, file)
        }
      )
